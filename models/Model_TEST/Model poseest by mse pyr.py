...
class AVATARModel(ModelBase):
    ...
    def onInitialize(self, batch_size=-1, **in_options):
        ...
        poseest = PoseEstimator(resolution, FaceType.toString(FaceType.FULL) ).model
        ...
        self.GA = modelify(AVATARModel.ResNet (bgr_shape[2], use_batch_norm, n_blocks=6, ngf=ngf, use_dropout=True))( [Input(bgr_shape), Input ( (3,) ), ])
        self.GB = modelify(AVATARModel.ResNet (bgr_shape[2], use_batch_norm, n_blocks=6, ngf=ngf, use_dropout=True))( [Input(bgr_shape), Input ( (3,) ), ])
        ...
        real_A0pyr = poseest(real_A0)[0]
        real_B0pyr = poseest(real_B0)[0]
        ...
        fake_B0 = self.GA([real_A0, real_B0pyr])
        fake_A0 = self.GB([real_B0, real_A0pyr])

        fake_B0pyr = poseest (fake_B0)[0]
        fake_A0pyr = poseest (fake_A0)[0]
        ...
        rec_A0 = self.GB ([fake_B0, real_A0pyr])
        rec_B0 = self.GA ([fake_A0, real_B0pyr])

        #import code
        #code.interact(local=dict(globals(), **locals()))

        loss_GA = DLoss(fake_B0_d_ones, fake_B0_d ) + \
                  lambda_A * 100 * K.mean(  K.square(fake_B0pyr-real_B0pyr)   ) + \
        ...
        loss_GB = DLoss(fake_A0_d_ones, fake_A0_d ) + \
                  lambda_B * 100 *K.mean( K.square(fake_A0pyr-real_A0pyr) ) + \
