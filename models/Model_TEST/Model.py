...
class AVATARModel(ModelBase):
    ...
    def onInitialize(self, batch_size=-1, **in_options):
        ...
        # self.enc = modelify(AVATARModel.DFEncFlow ())( [Input(bgr_shape), Input ( (3,) ), ] )
        ...
        self.GA = modelify(AVATARModel.UNet (bgr_shape[2], use_batch_norm, ngf=ngf, use_dropout=True))(Input(bgr_shape))
        self.GB = modelify(AVATARModel.UNet (bgr_shape[2], use_batch_norm, ngf=ngf, use_dropout=True))(Input(bgr_shape))
        ...
        real_rec_A0 = Input(bgr_shape)
        real_A0m = Input(mask_shape)
        real_B0 = Input(bgr_shape)
        real_rec_B0 = Input(bgr_shape)
        real_B0m = Input(mask_shape)

        real_A0m_blurred = gaussian_blur( max(1, K.int_shape(real_A0m)[1] // 32) ) (real_A0m)
        real_B0m_blurred = gaussian_blur( max(1, K.int_shape(real_B0m)[1] // 32) ) (real_B0m)
        ...
        if self.is_training_mode:
            ...
            self.GA_train = K.function ([real_A0, real_rec_A0, real_A0m, real_B0, real_rec_B0, real_B0m],[loss_GA],
                                        opt().get_updates(loss_GA, weights_GA) )

            self.GB_train = K.function ([real_A0, real_rec_A0, real_A0m, real_B0, real_rec_B0, real_B0m],[loss_GB],
            ...
            output_sample_types=[ {'types': (t.IMG_SOURCE, face_type, t.MODE_BGR), 'resolution':resolution},
                                  {'types': (t.IMG_SOURCE, face_type, t.MODE_BGR), 'resolution':resolution},
    ...
    def onTrainOneIter(self, generators_samples, generators_list):
        warped_src, src, srcm  = generators_samples[0]
        warped_dst, dst, dstm  = generators_samples[1]

        loss_GA, = self.GA_train ( [warped_src, src, srcm, warped_dst, dst, dstm] )
        loss_GB, = self.GB_train ( [warped_src, src, srcm, warped_dst, dst, dstm] )
        loss_DA, = self.DA_train( [warped_src, srcm, warped_dst, dstm] )
        loss_DB, = self.DB_train( [warped_src, srcm, warped_dst, dstm] )
    ...
    def onGetPreview(self, sample):
        ...
        test_A0m  = sample[0][2][0:4]

        test_B0   = sample[1][1][0:4]
        test_B0m  = sample[1][2][0:4]
