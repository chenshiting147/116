...
class Model(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        if is_first_run:
            self.options['lighter_ae'] = io.input_bool ("使用轻量级自动编码器? (y/n, ?:帮助 默认:n) : ", False, help_message="轻量级自动编码器更快，需要更少的VRAM，牺牲了整体质量。如果你的GPU VRAM <= 4，你应该选择这个选项。")
        else:
        ...
        if is_first_run or ask_override:
            ...
            self.options['pixel_loss'] = io.input_bool ("使用像素损失?  (y/n, ?:帮助 默认: n/default ) : ", def_pixel_loss, help_message="像素损失可能有助于增强精细细节和稳定面部颜色。只有在质量没有随着时间的推移而提高时才使用它。")
