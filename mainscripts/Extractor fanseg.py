...
def main(input_dir,
    ...
    if not input_path.exists():
        raise ValueError('未找到输入目录,请确认它存在.')
    ...
    if manual_output_debug_fix:
        if debug_dir is None:
            raise ValueError('必须指定调试目录')
        detector = 'manual'
        io.log_info('执行从调试目录中删除的帧的重新提取.')
    ...
    io.log_info('-------------------------')
    io.log_info('找到图片:        %d' % (images_found))
    io.log_info('人脸检测:        %d' % (faces_detected))
    io.log_info('-------------------------')
