...
class ConvertSubprocessor(Subprocessor):
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if (self.converter.type == Converter.TYPE_FACE or self.converter.type == Converter.TYPE_FACE_AVATAR) \
                ...
                if not self.debug:
                    self.log_info('没有为 %s 找到脸部,复制时没有脸部' % (filename_path.name))
                ...
                if self.converter.type == Converter.TYPE_IMAGE:
                    ...
                    if dflimg is not None:
                        ...
                        self.log_err("%s 不是一个dfl图像文件" % (filename_path.name))
                    ...
                    for face_num, image_landmarks in enumerate(faces):
                        try:
                            if self.debug:
                                self.log_info('\n将face_num [%d] 转换成文件 [%s]' % (face_num, filename_path))
                            ...
                            if 'MemoryError' in e_str:
                                ...
                                raise Exception('错误地将face_num [%d]转换成文件[%s]: %s' % (face_num, filename_path, e_str))
    ...
    # override
    def __init__(self, converter, input_path_image_paths, output_path, alignments, avatar_image_paths=None,
                 debug=False):
    ...
    def on_clients_initialized(self):
        if self.debug:
            io.named_window("调试转换")

        io.progress_bar("转换", len(self.input_data_idxs))
    ...
    def on_result(self, host_dict, data, result):
        if result[0] == 0:
            ...
            for img in result[1]:
                io.show_image('调试转换', (img * 255).astype(np.uint8))
                io.wait_any_key()
...
def main(args, device_args):
    ...
    try:
        ...
        if converter.type == Converter.TYPE_FACE or converter.type == Converter.TYPE_FACE_AVATAR:
            ...
            for filepath in io.progress_bar_generator(aligned_path_image_paths, "收集比对"):
                ...
                if dflimg is None:
                    io.log_err("%s 不是一个dfl图像文件" % (filepath.name))
                    continue
        ...
        if converter.type == Converter.TYPE_FACE_AVATAR:
            if avaperator_aligned_dir is None:
                io.log_err('未找到Avatar操作符对比的目录.请确保它存在.')
                return

            avaperator_aligned_path = Path(avaperator_aligned_dir)
            if not avaperator_aligned_path.exists():
                io.log_err('未找到Avatar操作符对比（aligned）的目录.请确保它存在.')
                return

            avatar_image_paths = []
            for filename in io.progress_bar_generator(Path_utils.get_image_paths(avaperator_aligned_path),
                ...
                if dflimg is None:
                    io.log_err("致命错误: %s 不是一个dfl图像文件" % (filepath.name))
                    return
